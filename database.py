...
####Travel time####
## Show all available path
import sys

def show_all_path(mode, usr_input):
    "Show all the path segments known to the database"
    ...
    try:
        sql = None
        path_dict = None
        # Try executing the SQL and get data from the database
        if mode == 1:
            print("\n Displayiong all show path segments \n")
            sql = """
            SELECT *
            FROM opaltravel.traveltimes;
            """
            cur.execute(sql, ())
            path_dict = dictfetchall(cur, sql,)
        else:
            print("Show specific path is called")
            print(usr_input)
            sql = """
            SELECT
                s1.stationname AS startstation,
                s2.stationname AS endstation,
                TT.expectedtraveltimeseconds,
                TT.stopstraversed,
                TT.triplegs,
                TT.coordinatemaplen
            FROM
                OpalTravel.TravelTimes TT
            JOIN OpalTravel.stations s1 ON TT.startstationid = s1.stationid
            JOIN OpalTravel.stations s2 ON TT.endstationid = s2.stationid
            WHERE TT.stopstraversed = %s;
            """
            cur.execute(sql, (usr_input,))
            path_dict = dictfetchall(cur, sql, (usr_input,))
        print("\n reached here \n")
        print("path_dict content:, ", path_dict)
        conn.commit()
    except Exception as e:
        # Handle specific exceptions and provide meaningful error messages or logging
...
def add_path_segment(startid, endid, expectedime, stopstraversed, triplegs, coordinatemaplen):
    "Add a new path segment to the database"
    # Data validation checks are assumed to have been done in route processing
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """
        INSERT INTO OpalTravel.TravelTimes (startstationid, endstationid,
        expectedtraveltimeseconds, stopstraversed, triplegs, coordinatemaplen)
        VALUES (%s, %s, %s, %s, %s, %s);
        """

        cur.execute(sql,(startid, endid, expectedime, stopstraversed, triplegs, coordinatemaplen))
...
def update_path_segment(startid, endid, expectedtime, stopstraversed, triplegs, coordinatemaplen):
    "Update other fields for a particular path segment"
    # Get the database connection and set up the cursor
    conn = database_connect()
    if(conn is None):
        # If a connection cannot be established, send an Null object
    ...
    try:
        ...
        if expectedtime is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "expectedtime = %s::bigint\n"
            attcounter += 1
        if stopstraversed is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "stopstraversed = %s\n"
            attcounter += 1
        if triplegs is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "triplegs = %s::bigint\n"
            attcounter += 1
        if coordinatemaplen is not None:
            if attcounter != 0:
                setitems += ","
            setitems += "coordinatemaplen = %s::bigint\n"
            attcounter += 1

        # Retrieve all the information we need from the query
        sql = f"""UPDATE OpalTravel.TravelTimes
                    SET {setitems}
                    WHERE startstationid = 1
                    AND endstationid = 2;"""
        print_sql_string(sql,(startid, endid, expectedtime, stopstraversed, triplegs, coordinatemaplen))
        val = dictfetchone(cur,sql,(startid, endid, expectedtime, stopstraversed, triplegs, coordinatemaplen))
        conn.commit()
...
def delete_path_segment(startid, endid):
    """
    Remove a path segment from your system
    """
    # Data validation checks are assumed to have been done in route processing
    conn = database_connect()
    if(conn is None):
    ...
    try:
        # Try executing the SQL and get from the database
        sql = """
        DELETE FROM OpalTravel.TravelTimes
        WHERE startstationid = %s
        AND endstationid = %s;
        """

        cur.execute(sql,(startid, endid))
        conn.commit()                   # Commit the transaction
        r = []
        # r = cur.fetchone()
        # print("return val is:")
        ...
        # If there were any errors, return a NULL row printing an error to the debug
        print("Unexpected error deleting path segment")
        cur.close()                     # Close the cursor
        conn.close()                    # Close the connection to the db
        raise
...
def produce_report():
    "Produce a report showing the various stations alongside with the longest expected travel time"
    ...
    try:
        ...
        path_dict = None
        # Try executing the SQL and get data from the database
        print("\n Displayiong all report \n")
        sql = """
        SELECT s.stationname AS StationName, MAX(tt.expectedtraveltimeseconds) AS LongestExpectedTravelTime
        FROM opaltravel.stations s
        INNER JOIN opaltravel.traveltimes tt ON s.stationid = tt.startstationid
